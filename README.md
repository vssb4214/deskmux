...
- **Peripheral sharing isn't here yet.** DeskMux switches *displays* today. Keyboard/mouse sharing and smart cursor-driven handoff are first-class **future goals** (see [Roadmap](#roadmap)) — and worth being precise about what's physically possible:
  - *Software sharing* (control another machine's cursor over the network, with the keyboard following the active machine) is on the roadmap as built-in DeskMux functionality where feasible — not implemented yet.
...
- [ ] Cursor-driven focus handoff: keyboard/mouse follow the active machine; monitor presets can switch with focus (e.g. cursor crosses from Windows screen to Mac screen)
- [ ] Smart handoff triggers: cursor crossing, hotkeys, tray actions, focus changes
