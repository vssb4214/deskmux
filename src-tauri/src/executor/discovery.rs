//! In-app monitor discovery: enumerate connected displays and read the current VCP 0x60
...
//! diagnostic session. Read-only by design; probe-writes are deliberately not exposed here
...
mod tests {
    ...
    impl ScriptedController {
        fn new(displays: Vec<&str>, get_vcp_results: Vec<io::Result<VcpReading>>) -> Self {
            Self {
    ...
    impl NativeDdcController for ScriptedController {
        ...
        fn set_vcp_feature(&self, _: &str, _: u8, _: u16) -> io::Result<()> {
    ...
    fn reads_current_input_on_first_attempt() {
        let controller = ScriptedController::new(vec!["K@P:d0e5:0"], vec![reading(4626, 4626)]);
    ...
    fn missing_display_is_not_found_and_never_retried() {
        let controller = ScriptedController::new(vec!["OTHER:0"], vec![not_found("GHOST:0000:0")]);
    ...
    fn list_maps_discovered_displays() {
        let controller = ScriptedController::new(vec!["K@P:d0e5:0", "KJL:0e25:2"], vec![]);
